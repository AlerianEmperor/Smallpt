...
//http://fastcpp.blogspot.fr/2011/03/changing-sign-of-double-values-using-sse.html
...
const double invtwopi = 0.1591549;
const double twopi = 6.283185;
const double threehalfpi = 4.7123889;
const double pi = 3.141593;
const double halfpi = 1.570796;
const double quarterpi = 0.7853982;
...
double FT::length(double x, double y)
{
	return FT::sqrt(x*x + y*y);
}
double FT::length(double x, double y, double z)
{
	return FT::sqrt(x*x + y*y + z*z);
}

double FT::atan(double x)
{
	return quarterpi*x - x*(fabs(x) - 1)*(0.2447 + 0.0663*fabs(x));
}
double FT::atan2(double y, double x)
{
	if (fabs(x)>fabs(y)) {
		double atan = FT::atan(y / x);
		if (x>0.f)
			return atan;
		else
...
double FT::sin(double angle) {
	return FT::cos(halfpi - angle);
}
void FT::sincos(double angle, double *sin, double *cos) {
	//clamp to the range 0..2pi
	angle = angle - floorf(angle*invtwopi)*twopi;
	double sinmultiplier = angle>0.f&&angle<pi ? 1.f : -1.f;
	angle = angle>0.f ? angle : -angle;
...
__m128 FT::atan_ps(__m128 x)
{
	...
	return _mm_sub_ps(_mm_mul_ps(_mm_set1_ps(quarterpi), x),
		...
		(_mm_add_ps(_mm_set1_ps(0.2447), _mm_mul_ps(_mm_set1_ps(0.0663), _mm_andnot_ps(SIGNMASK, x))))));
}
...
{
	const __m128 c1 = _mm_set1_ps(0.99940307);
	const __m128 c2 = _mm_set1_ps(-0.49558072);
	const __m128 c3 = _mm_set1_ps(0.03679168);
...
//QT
#ifdef QT_GUI_LIB
double FT::length(QVector2D vector) {
	return FT::length(vector.x(), vector.y());
}
double FT::length(QVector3D vector) {
	return FT::length(vector.x(), vector.y(), vector.z());
}
double FT::angle(QVector2D vector) {
	return FT::atan2(vector.y(), vector.x());
}
double FT::azimuth(QVector3D vector) {
	return FT::atan2(vector.y(), vector.x());
}
double FT::inclination(QVector3D vector) {
	double projectedlength = FT::length(vector.toVector2D());
	return FT::atan2(vector.z(), projectedlength);
}
double FT::x(QVector2D vector) {
	return vector.x()*FT::cos(vector.y());
}
double FT::y(QVector2D vector) {
	return vector.x()*FT::sin(vector.y());
}
double FT::x(QVector3D vector) {
	return vector.x()*FT::cos(vector.y())*FT::cos(vector.z());
}
double FT::y(QVector3D vector) {
	return vector.x()*FT::sin(vector.y())*FT::cos(vector.z());
}
double FT::z(QVector3D vector) {
...
QVector3D FT::spherical2cartesian(QVector3D vector) {
	double azimuthsin, azimuthcos;
	FT::sincos(vector.y(), &azimuthsin, &azimuthcos);
	double inclinationsin, inclinationcos;
...
{
	double atan(double x);
	double cos_52s(double x);

	__m128 atan_ps(__m128 x);
	__m128 cos_52s_ps(__m128 x);
};

double FTA::sqrt(double squared)
{
	return _mm_cvtss_f32(_mm_sqrt_ss(_mm_set_ss(squared)));
}
double FTA::length(double x, double y)
{
	return FTA::sqrt(x*x + y*y);
}
double FTA::length(double x, double y, double z)
{
	return FTA::sqrt(x*x + y*y + z*z);
}

double FTA::atan(double x)
{
	double u = x*x;
	double u2 = u*u;
	double u3 = u2*u;
	double u4 = u3*u;
	double f = 1.f + 0.33288950512027*u - 0.08467922817644*u2 + 0.03252232640125*u3 - 0.00749305860992*u4;
	return x / f;
}
double FTA::atan2(double y, double x)
{
	if (fabs(x)>fabs(y)) {
		double atan = FTA::atan(y / x);
		if (x>0.f)
			return atan;
		else
			return y>0.f ? atan + pi : atan - pi;
...
double FTA::sin(double angle) {
	return FTA::cos(halfpi - angle);
}
void FTA::sincos(double angle, double *sin, double *cos) {
	//clamp to the range 0..2pi
	angle = angle - floorf(angle*invtwopi)*twopi;
	double sinmultiplier = angle>0.f&&angle<pi ? 1.f : -1.f;
	angle = angle>0.f ? angle : -angle;
...
__m128 FTA::atan_ps(__m128 x)
{
	...
	//__m128 f=1.f+0.33288950512027*u-0.08467922817644*u2+0.03252232640125*u3-0.00749305860992*u4;

	__m128 f = _mm_add_ps(_mm_add_ps(_mm_add_ps(_mm_add_ps(_mm_set1_ps(1.f),
		_mm_mul_ps(_mm_set1_ps(0.33288950512027), u)),
		_mm_mul_ps(_mm_set1_ps(-0.08467922817644), u2)),
		_mm_mul_ps(_mm_set1_ps(0.03252232640125), u3)),
		_mm_mul_ps(_mm_set1_ps(-0.00749305860992), u4));
...
{
	const __m128 c1 = _mm_set1_ps(0.9999932946);
	const __m128 c2 = _mm_set1_ps(-0.4999124376);
	const __m128 c3 = _mm_set1_ps(0.0414877472);
	const __m128 c4 = _mm_set1_ps(-0.0012712095);
	__m128 x2;      // The input argument squared
	x2 = _mm_mul_ps(x, x);
...
//QT
#ifdef QT_GUI_LIB
double FTA::length(QVector2D vector) {
	return FTA::length(vector.x(), vector.y());
}
double FTA::length(QVector3D vector) {
	return FTA::length(vector.x(), vector.y(), vector.z());
}
double FTA::angle(QVector2D vector) {
	return FTA::atan2(vector.y(), vector.x());
}
double FTA::azimuth(QVector3D vector) {
	return FTA::atan2(vector.y(), vector.x());
}
double FTA::inclination(QVector3D vector) {
	double projectedlength = FTA::length(vector.toVector2D());
	return FTA::atan2(vector.z(), projectedlength);
}
double FTA::x(QVector2D vector) {
	return vector.x()*FTA::cos(vector.y());
}
double FTA::y(QVector2D vector) {
	return vector.x()*FTA::sin(vector.y());
}
double FTA::x(QVector3D vector) {
	return vector.x()*FTA::cos(vector.y())*FTA::cos(vector.z());
}
double FTA::y(QVector3D vector) {
	return vector.x()*FTA::sin(vector.y())*FTA::cos(vector.z());
}
double FTA::z(QVector3D vector) {
