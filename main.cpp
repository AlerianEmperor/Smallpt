...
void main()
{
	...
	double tan_theta = tanf(60.0f);//tanf(40.0 * pi / 180.0);
	...
	for (int j = 0; j < Height; ++j)
	{
		...
		for (int i = 0; i < Width; ++i)
		{
			...
			c[j * Width + i] = color * ins;
		}
	}
	std::cout << "\nRendering time: " << ((double)clock() - t_render) / CLOCKS_PER_SEC << "\n";

	ofstream ofs("2022_2.ppm");
	...
	cout << "Write Time: " << ((double)clock() - t_write) / CLOCKS_PER_SEC << "\n";
	getchar();
}
