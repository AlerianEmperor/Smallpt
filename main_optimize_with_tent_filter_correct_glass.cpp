...
struct Sphere
{
	double rad_square;
	vec3 p, c;// , e;
	unsigned int mtl_type = Diffuse;
	//Sphere(double rad_, vec3 p_, vec3 c_, vec3 e_, int mtl_type_) : rad_square(rad_ * rad_), p(p_), c(c_), e(e_), mtl_type(mtl_type_) {}
	Sphere(double rad_, vec3 p_, vec3 c_, vec3 e_, int mtl_type_) : rad_square(rad_ * rad_), p(p_), c(c_), mtl_type(mtl_type_) {}
...
const Sphere spheres[] = {//Scene: radius, position, color, emission, material
	...
	Sphere(600.0, vec3(50.0, 681.6 - 0.27 ,81.6), vec3(0.0, 0.0, 0.0), vec3(12, 12, 12), Light) //Lite
};
...
static vec3 Radiance(const Ray& r, const int& num_spheres)
{
	...
	for (int z = 0; z <= 10; ++z)
	{
		...
		if (intersect(new_ray, num_spheres, t, id))
		{
			...
			//all out going ray are normalize so we dont need norm any way
			if (spheres[id].mtl_type & Light)
			{
				L += T * LIGHT_POWER;//spheres[id].e;
				return L;
			}
			else if (spheres[id].mtl_type & Diffuse)
			{
				...
				vec3 direction_out(r2s * (u * c + v * s) + normal * sqrt14(1.0 - r1));//this ray is already norm
				...
				new_ray = Ray(hit_point + 0.02 * normal, (new_ray.d - 2.0 * new_ray.d.dot(normal) * normal));
			else if (spheres[id].mtl_type & Glass)
			{
				...
				else
				{
					...
					Refractive_direction = (eta * new_ray.d + (eta * abs_cos_i - sqrt14(c2)) * real_normal);
					double cos_t = abs(Refractive_direction.dot(normal));
					//double cos_t = cos_i >= 0 ? Refractive_direction.dot(normal) : -Refractive_direction.dot(normal);
				...
				if (randf() < P)
				{
					new_ray = Ray(hit_point + 0.02 * real_normal, (new_ray.d - 2.0 * new_ray.d.dot(real_normal) * real_normal));
					//if (z > 2)
					T = T * ((R / P));
					//else
					//	T = T / R;
...
void main()
{
			...
			for (int sx = 0; sx < 2; ++sx)
			{
				...
				{		
					...
					for (int s = 0; s < ns; ++s)
					{
						...
						double p = ((sx + 0.5 + r1) * 0.5 + i) * iWidth - 0.5;
						double q = 0.5 - ((sy + 0.5 + r2) * 0.5 + j) * iHeight;
					
						Ray r(origin, (u * p + v * q + w).norm());
						sum = sum + Radiance(r, num_spheres);
					}
					c[j * Width + i] += sum * ins * 0.25f;//vec3((sum.x), (sum.y), (sum.z)) * 0.25;
				}
			}
	...
	string name = "a_Final_8192";
	//string name = "Glass_Change_R_To_P_In_Relfection_Selection";
	//string name = "Glass_No_Multiply_For_Reflection";
	//string name = "a_test_original_divide_smallpt_no_clampf";
	//string name = "Final_Glass_original_divide";
	//string name = "Final_Glass_ior_1.5_40_spp_depth_2_multiply_larger_divide_smaller_after_sum";
